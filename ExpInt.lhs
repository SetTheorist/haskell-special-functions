...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Exponential integral $\O{Ei}$}
The exponential integral $\O{Ei} z$ is defined for $x<0$ by
\[ \O{Ei}(z) = -\int_{-x}^\infty \frac{e^{-t}}{t}\,dt \]
It can be defined 
...
\begin{titled-frame}{$\text{\color{blue}\tt sf\_expint\_ei z} = \O{Ei}(z)$\marginnote{\tt sf\_expint\_ei}}
\begin{code}
...
\[ \O{Ei}(x) = \gamma + \ln x + \sum_{n=1}^\infty \frac{x^n}{n! n} \marginnote{$\O{Ei}(x)$} \]
...
\[ \O{Ei}(x) \sim \frac{e^x}{x}\sum_{n=0}^\infty \frac{n!}{x^n} \]
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Exponential integral $\O{E}_n$}
The exponential integrals $\O{E}_n(z)$ are defined for $n=0,1,\dots$ and $\Re z>0$ via
\[ \O{E}_n(z) = z^{n-1}\int_{z}^\infty \frac{e^{-t}}{t^n}\,dt \marginnote{$\O{E}_n(z)$} \]
They satisfy the following relations:
\begin{eqnarray*}
\O{E}_0(z) &=& \frac{e^{-z}}{z} \\
\O{E}_{n+1}(z) &=& \int_z^\infty \O{E}_{n}(t)\,dt \\
\end{eqnarray*}
And they can be expressed in terms of incomplete gamma functions:
\[ \O{E}_n(z) = z^{n-1}\Gamma(1-n,z) \]
(which also gives a generalization for non-integer $n$).

\subsection{\tt sf\_expint\_en n z}
We evaluate the exponential integrals $\O{E}_n(z)$ by handling the special cases
$n=0,1$ directly, otherwise use a series expansion for $|z|\leq1$
and a continued fraction expansion otherwise.
\begin{titled-frame}{$\text{\color{blue}\tt sf\_expint\_en n z} = \O{E}_n(z)$}
\begin{code}
...
We use this series expansion for $\O{E}_1(z)$:
\[ \O{E}_1(z) = -\gamma - \ln z + \sum_{k=1}^\infty(-)^k\frac{z^k}{k! k}\]
...
\[ \O{E}_n(z) = \frac{(-z)^{n-1}}{(n-1)!}(-\ln(z) + \psi(n)) - \sum_{m=0,m\neq n}^\infty \frac{(-x)^m}{(m-(n-1))m!} \]
for $n\geq2$, $z\leq1$
